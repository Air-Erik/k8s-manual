...
**Этап 0.1.1 — Исследование (START HERE):**
  ...
  - [ ] AI-агент создаёт опросник: `research/nsx-analysis/04-current-config-questionnaire.md`
...
**Этап 0.1.4 — Валидация:**
  - [ ] AI-агент создаёт чек-лист: `research/nsx-analysis/09-validation-checklist.md`
...
- 🟡 Этап 0: Подготовка инфраструктуры — **IN PROGRESS** (5% — задача для AI-агента подготовлена)
- 🔴 Этап 1: Bootstrap кластера — **TODO** (0%)
- 🔴 Этап 2: Production-готовность — **TODO** (0%)
- 🔴 Этап 3: Миграция — **FUTURE** (0%)

### Следующие шаги (СЕЙЧАС):
1. **Оператор** передаёт задачу AI-агенту (см. инструкцию ниже) ⬅️ **ВЫ ЗДЕСЬ**
2. **AI-агент** создаёт образовательные материалы по NSX-T
3. **Оператор** изучает материалы и исследует текущую NSX-T конфигурацию
4. **AI-агент** анализирует данные и даёт рекомендации
5. **AI-агент** создаёт пошаговые инструкции
6. **Оператор** применяет инструкции в NSX-T
7. Переход к задаче 0.2 (VM Preparation)
